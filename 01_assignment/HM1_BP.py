...
if __name__ == "__main__":
    ...
    for i in range(10):
        input_vector[i, :] = read_img("mnist_subset/" + str(i) +
                                      ".png").reshape(-1) / 255.
    gt_y = np.zeros((10, 1))
    gt_y[0] = 1
    ...
    for i in range(50):
        ...
        loss = -(gt_y * np.log(pred_y) +
                 (1 - gt_y) * np.log(1 - pred_y)).sum()  #cross-entroy loss
        ...
        grad_output_layer_2 = grad_pred_y * (1 - pred_y)
        grad_layer_2 = output_layer_1_act.T @ grad_output_layer_2
        grad_output_layer_1_act = grad_output_layer_2 @ MLP_layer_2.T
        grad_output_layer_1 = grad_output_layer_1_act * (
            output_layer_1_act) * (1 - output_layer_1_act)
        grad_layer_1 = input_vector.T @ grad_output_layer_1
