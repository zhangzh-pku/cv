...
def corner_response_function(I_xx, I_yy, I_xy, window_size, alpha, threshold):
    ...
    corner_map = I_xx * I_yy - I_xy**2 - alpha * ((I_xx + I_yy)**2)
    Rmax = np.max(corner_map)
    index = np.where(corner_map > threshold * Rmax)
    theta = corner_map[index]
    corner_list = tuple(zip(index[0], index[1], theta))
...
if __name__ == "__main__":
    ...
    corner_list = corner_response_function(I_xx, I_yy, I_xy, window_size,
                                           alpha, threshold)
